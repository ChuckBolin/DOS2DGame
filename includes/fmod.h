...
enum FSOUND_OUTPUTTYPES
{
    ...
    FSOUND_OUTPUT_NOSOUND_NONREALTIME   /* This is the same as nosound, but the sound generation is driven by FSOUND_update(double timeDifference) */
};
...
#define FSOUND_INIT_STREAM_FROM_MAIN_THREAD 0x0200    /* Turns off fmod streamer thread, and makes streaming update(double timeDifference) from FSOUND_update(double timeDifference) called by the user */
/* [DEFINE_END] */
...
DLL_API void            F_API FSOUND_update();   /* This is called to update(double timeDifference) 3d sound / non-realtime output */
...
DLL_API int             F_API FSOUND_DSP_GetBufferLength();      /* Length of each DSP update(double timeDifference) */
